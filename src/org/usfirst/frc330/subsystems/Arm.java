...
public class Arm extends Subsystem implements LiveWindowSendable {
    ...
    private CANTalon activeSide = armL;
...
//		Robot.csvLogger.add("ArmTicks", temp);
	...
	{
		return (-convertRotationsToDegrees(activeSide.getPosition()));
	}
	...
	{
		return (convertTicksToDegrees(activeSide.getForwardSoftLimit()));
	}
    
    public double getUpperLimit()
	{
		return (convertTicksToDegrees(activeSide.getReverseSoftLimit()));
	}

	public double getArmOutput() {
		return activeSide.getOutputVoltage()/activeSide.getBusVoltage();
	}
	
	public boolean getLowerLimitTripped(){
		if (activeSide.getFaultForSoftLim() != 0)
			return true;
		else
			return false;
	}
	...
	public boolean getUpperLimitTripped(){
		if(activeSide.getFaultRevSoftLim() != 0)
			return true;
		else
			return false;
	}
	...
	public boolean getSensorFault(){
    	...
    	sensorStatus = activeSide.isSensorPresent(CANTalon.FeedbackDevice.CtreMagEncoder_Relative);
    ...
    public void setArm(double output) {
    	...
    	activeSide.set(output);
    }
    
    /* Set the arm angle */
    public void setArmAngle(double position) {
    	changeControlMode(TalonControlMode.Position);
    	activeSide.setSetpoint(convertDegreesToRotations(position));
    	if (SCtable != null)
    		SCtable.putNumber("setpoint", position);
    }
    
    public double getSetpoint() {
    	return convertRotationsToDegrees(-activeSide.getSetpoint());
    }
    
    public void setPIDConstants (double P, double I, double D)
	{
		activeSide.setPID(P, I, D);
		if (SCtable != null) {
    ...
    public void setMaxOutput(double maxOutput){
    	activeSide.configMaxOutputVoltage(maxOutput);
    ...
    public void setQuadrantPosition(int quadrant)
    {
    	...
    	if ( quadrant >= ArmConst.minQuadrant && quadrant <= ArmConst.maxQuadrant )
    	{
    		...
    		activeSide.setPosition(convertDegreesToRotations(currentArmAngle + angleChange));
    		if (activeSide.getControlMode() == TalonControlMode.Position)
    ...
    public void manualArm() {	
    	...
    	else if ( activeSide.getControlMode() != TalonControlMode.Position) {
			angle = getArmAngle();
    ...
    public boolean onArmTarget() {
    	double error = convertTicksToDegrees(activeSide.getClosedLoopError());
	...
	{        
        ...
        if (Robot.isPracticeRobot())
            name = "PracticeArmZeroRed";
        else
            name = "CompetitionArmZeroRed";
        Preferences.getInstance().putInt(name, armR.getPulseWidthPosition());
    ...
    private void setLowerSoftLimit(double lowerAngle) {
    	activeSide.setForwardSoftLimit(convertDegreesToRotations(lowerAngle));
    }
    
    private void setUpperSoftLimit(double upperAngle) {
    	activeSide.setReverseSoftLimit(convertDegreesToRotations(upperAngle));
    }
    ...
    public void initTable(ITable table) {
        ...
        if(table != null) {
            ...
            table.putNumber("Mode", activeSide.getControlMode().getValue());
            table.putNumber("p", activeSide.getP());
            table.putNumber("i", activeSide.getI());
            table.putNumber("d", activeSide.getD());
            table.putNumber("f", activeSide.getF());
	...
	private final ITableListener listener = new ITableListener() {
		...
		public void valueChanged(ITable table, String key, Object value, boolean isNew) {
			if (key.equals("p") || key.equals("i") || key.equals("d") || key.equals("f")) {
				if (activeSide.getP() != table.getNumber("p", 0.0) || activeSide.getI() != table.getNumber("i", 0.0)
						|| activeSide.getD() != table.getNumber("d", 0.0) || activeSide.getF() != table.getNumber("f", 0.0))
					Robot.logger.println("Changing PID from SmartDashboard.");
					Robot.logger.println("Old Value P: " + activeSide.getP() + " New Value: " + table.getNumber("p", 0.0));
					Robot.logger.println("Old Value I: " + activeSide.getI() + " New Value: " + table.getNumber("i", 0.0));
					Robot.logger.println("Old Value D: " + activeSide.getD() + " New Value: " + table.getNumber("d", 0.0));
					Robot.logger.println("Old Value F: " + activeSide.getF() + " New Value: " + table.getNumber("f", 0.0));
				...
				if (activeSide.getControlMode().getValue() != ((Double) value).intValue()) {
	...
	public void changeControlMode(TalonControlMode newControlMode) {
		TalonControlMode oldControlMode = activeSide.getControlMode();
		...
		if (oldControlMode != newControlMode) {
			...
			activeSide.changeControlMode(newControlMode);
